...
function createMines(minescount, i, j) {
	...
	for (var idx = 0; idx < minescount; idx++) {
		while (gBoard[minePos.i][minePos.j] === gBoard[i][j]) {
...
function sweepAround(rowIdx, colIdx) {
	...
	if (gBoard[rowIdx][colIdx].isMine) {
		gPrevCellList.push(gBoard[rowIdx][colIdx])
		return mineClicked(elCurCell)
	}
	if (gBoard[rowIdx][colIdx].minesAround !== 0) {
		if (!gPrevCellList.length) gPrevCellList.push(gBoard[rowIdx][colIdx])
...
function mineClicked(elCurrCell) {
	...
	elCurrCell.style["background-color"] = "#b11919"
	gGame.lives -= 1
	gLivesLost++
...
function renderHit(lives) {
	...
	if (lives > 0) {
		elSmiley.style.background = "#f3f3f3 url(img/Frowney.png) no-repeat right top"
		elSmiley.style["background-size"] = '65px 65px'
		setTimeout(() => { renderSmiley(elSmiley) }, 3000);
	} else {
		elSmiley.style.background = "#f3f3f3 url(img/Explosion.png) no-repeat right top"
		elSmiley.style["background-size"] = '65px 65px'
	}
...
function renderSmiley(elSmiley) {
	elSmiley.style.background = "#f3f3f3 url(img/Smiley-none-transparent.png) no-repeat right top"
	elSmiley.style["background-size"] = '65px 65px'
}
